...
abstract class Daemon(val context: Context, var d: Dashboard) : IdGenerator.Indexed {
    ...
    companion object {
        inline operator fun invoke(context: Context, dashboard: Dashboard, type: Type):Daemon {
            return when (type) {
                Type.MQTTD -> Mqttd(context, dashboard)
                Type.BLUETOOTHD -> Bluetoothd(context, dashboard)
            }
        }
